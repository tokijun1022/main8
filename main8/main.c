...
//  fft部分も並列化させたもの。k.pの不具合も修正済み。読み込みの際の不具合も修正済み（これにより、完璧な一様系の再現に成功。)。さらにEc以下のみの要素のみ読み込み、計算することでメモリー及び計算時間の大幅な節約を施したもの。さらに、精度の向上と高速化のため、BdGプログラムでのシステムの大きさを倍にして計算するようにしたもの。2015/11/25
